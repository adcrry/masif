...
for ppi_pair_id in ppi_pair_ids:
    ...
    for ix, pid in enumerate(pids):
        ...
        tic = time.time()
        embeddings_ref = extract_masif_site_embeddings(
            params,
            learning_obj,
            rho_wrt_center,
            theta_wrt_center,
        ...
        for i in range(20):
            ...
            embeddings_rotated = extract_masif_site_embeddings(
                params,
                learning_obj,
                rho_wrt_center,
                np.mod(theta_wrt_center +random_angle, 2*np.pi),
                input_feat,
                mask,
                indices,
            )
            np.save(f"output/rotated{i}_embeddings_{pdb_chain_id}.npy", embeddings_rotated[0])
            np.save(f"output/rotated{i}_fc2_embeddings_{pdb_chain_id}.npy", embeddings_rotated[2])
            np.save(f"output/rotated{i}_score_values_{pdb_chain_id}.npy", embeddings_rotated[3])

        print("embedding_shape", embeddings_ref[0][0].shape)
        print("embedding_ref[0]", embeddings_ref[0][0])
        print("embedding_rotated[0]", embeddings_rotated[0][0])

        print('diff, theta=', random_angle, embeddings_ref[0][0] - embeddings_rotated[0][0])        
